...
func main() {
	...
	for i := 0; i < 6; i++ {
		...
		wg.Add(1)
		go handleSprocDetails(defDir, sprocCh, tablesCh, errCh, wg)
	}
...
func handleSprocDetails(defDir string, inCh <-chan keyValue, outCh chan<- []string, errCh chan<- []string, done *sync.WaitGroup) {
	for s := range inCh {
...
// defintion
func parseSproc(sproc keyValue) (errors, tables []string) {
...
func NewTreeShapeListener(ch chan<- string) *TreeShapeListener {
...
func (l *TreeShapeListener) ExitTsql_file(ctx *parser.Tsql_fileContext) {
	...
	for table := range l.info.Tables {
		...
		if strings.Contains(table, ".") {
			// no need ot check the whitelist -- this table refers to a DB other than BRS
